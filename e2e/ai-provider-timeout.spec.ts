...
test.describe('AI Provider Timeout Handling', () => {
  ...
  test('should recover after provider transient failure', async ({ page }) => {
    ...
    await page.route('**/api/chat', async (route: Route) => {
      callCount++;
      if (callCount === 1) { await route.fulfill({ status: 503, body: JSON.stringify({ error: 'transient' }) }); }
      else { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ reply: 'Coba keripik pedas atau balado, kak!', intent: 'recommend', components: [{ type: 'product_card', productId: 1 }] }) }); }
    });
    await page.goto('/pesan', { waitUntil: 'domcontentloaded', timeout: 30000 });
