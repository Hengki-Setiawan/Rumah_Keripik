...
export function PaymentInstructionCard({ amount, instruction }: Props) {
  ...
  const isGateway = instruction.provider === 'midtrans';
  ...
  return (
    <section className="mt-6 rounded-[1.5rem] border border-[#e5e7eb] bg-[#f7f7f8] p-5 text-[#111827]">
      ...
      <ul className="mt-4 space-y-2 text-sm text-[#6b7280]">
        ...
        {!isCod && isGateway && <li>2. Pindai kode QRIS di atas untuk membayar langsung.</li>}
        {!isCod && isGateway && <li>3. Setelah berhasil dibayar, status pesanan akan terupdate otomatis.</li>}
