  ...
  return (
    <main className="min-h-screen bg-neutral-100 p-4 text-neutral-950 print:bg-white print:p-0">
      ...
      <div className="no-print mx-auto mb-4 flex max-w-4xl items-center justify-between gap-3">
        <Link href="/pembayaran/verifikasi" className="rounded-xl border bg-white px-4 py-2 text-sm font-bold shadow-sm">
          Kembali
        </Link>
        <PrintDocumentButton orderId={order.id_transaksi} documentType={type} />
      </div>

      <section className={`print-card mx-auto max-w-4xl rounded-3xl bg-white p-8 shadow-xl ${isPackingLabel ? 'max-w-2xl' : ''}`}>
        <header className="flex items-start justify-between gap-6 border-b-2 border-neutral-900 pb-5">
          <div>
            <p className="text-sm font-black uppercase tracking-[0.22em] text-amber-700">Rumah Keripik</p>
            <h1 className="mt-2 text-3xl font-black tracking-tight">{TITLES[type]}</h1>
          ...
          <div className="text-right">
            ...
            <p className="text-xl font-black">{orderCode}</p>
            {data.document && (
              <>
                <p className="mt-2 text-xs font-bold uppercase text-neutral-500">No Dokumen</p>
                <p className="text-sm font-black">{data.document.document_number}</p>
              </>
            )}
        ...
        {!canShowReceipt && (
          <div className="mt-6 rounded-2xl border border-amber-300 bg-amber-50 p-4 text-sm font-bold text-amber-800">
        ...
        <section className="mt-6 grid gap-4 sm:grid-cols-2">
          <div className="rounded-2xl border p-4">
            <p className="text-xs font-black uppercase tracking-widest text-neutral-500">Penerima</p>
            <p className="mt-2 text-lg font-black">{order.nama_penerima || '-'}</p>
          ...
          {!isPackingLabel && (
            <div className="rounded-2xl border p-4">
              <p className="text-xs font-black uppercase tracking-widest text-neutral-500">Pembayaran</p>
              <p className="mt-2 text-lg font-black">{getPaymentMethodLabel(data)}</p>
        ...
        {isPackingLabel && (
          <section className="mt-6 rounded-2xl border-2 border-dashed border-neutral-900 p-5">
            <p className="text-xs font-black uppercase tracking-widest text-neutral-500">Instruksi Packing</p>
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id} className="border-b last:border-b-0">
                  ...
                  <td className="p-3 text-center font-black">{item.qty_terjual}</td>
                  {!isPackingLabel && <td className="p-3 text-right">{renderMoney(item.harga_snapshot)}</td>}
                  {!isPackingLabel && <td className="p-3 text-right font-black">{renderMoney(item.subtotal)}</td>}
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {!isPackingLabel && (
          <section className="mt-6 flex justify-end">
            <div className="w-full max-w-sm rounded-2xl bg-neutral-950 p-5 text-white">
              <div className="flex justify-between gap-4 text-sm">
                <span>Total Bayar</span>
                <span className="text-2xl font-black">{renderMoney(order.total_bayar)}</span>
              </div>
            </div>
          </section>
        )}
