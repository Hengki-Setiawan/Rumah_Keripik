...
export function HubKomunikasiV3Client({ initialSessions }: { initialSessions: SessionRow[] }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 xl:min-h-[680px] xl:grid-cols-[320px_minmax(0,1fr)_300px] 2xl:grid-cols-[360px_minmax(0,1fr)_340px]">
        <section className="rounded-2xl border border-outline-variant bg-surface-container-lowest">
          ...
          <div className="max-h-[420px] overflow-y-auto xl:max-h-[590px]">
        ...
        <section className="flex min-w-0 flex-col rounded-2xl border border-outline-variant bg-surface-container-lowest">
          <div className="flex flex-col gap-3 border-b border-outline-variant p-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0"><h2 className="text-lg font-semibold tracking-[-0.02em] text-on-surface">Percakapan</h2><p className="truncate text-sm text-on-surface-variant">{selected?.customerName || selected?.title || selectedId || 'Pilih chat'}</p></div>
            <div className="flex flex-wrap gap-2"><button onClick={() => setMode('manual')} className="inline-flex min-h-10 items-center gap-1 rounded-xl bg-primary px-3 py-2 text-xs font-medium text-on-primary"><Pause size={14} /> Manual</button><button onClick={() => setMode('enabled')} className="inline-flex min-h-10 items-center gap-1 rounded-xl bg-[#10a37f] px-3 py-2 text-xs font-medium text-white"><Play size={14} /> AI</button></div>
          </div>
          <div className="flex min-h-[360px] max-h-[60vh] flex-1 flex-col gap-3 overflow-y-auto bg-surface p-3 sm:p-4 xl:max-h-[470px]">
          ...
          <div className="space-y-3 border-t border-outline-variant bg-white p-3 sm:p-4">
            <div className="flex gap-2"><input value={message} onChange={(event) => setMessage(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') sendMessage(); }} placeholder="Balas manual sebagai admin..." className="min-w-0 flex-1 rounded-xl border border-outline-variant px-3 py-2 text-sm outline-none focus:border-primary/30" /><button onClick={sendMessage} className="rounded-xl bg-primary px-4 py-2 text-on-primary"><Send size={17} /></button></div>
            <div className="grid gap-2 lg:grid-cols-[180px_minmax(0,1fr)_auto]"><select value={cardType} onChange={(event) => setCardType(event.target.value as typeof cardType)} className="rounded-xl border border-outline-variant px-3 py-2 text-sm"><option value="quick_replies">Quick Replies</option><option value="product_cards">Product Cards</option><option value="location_picker">Location Picker</option><option value="payment_methods">Payment Methods</option><option value="order_status_card">Order Status</option></select><input value={cardPayload} onChange={(event) => setCardPayload(event.target.value)} placeholder="Labels/Product IDs pisahkan koma" className="min-w-0 rounded-xl border border-outline-variant px-3 py-2 text-sm outline-none focus:border-primary/30" /><button onClick={sendCard} className="rounded-xl border border-outline-variant px-4 py-2 text-sm font-medium text-on-surface hover:bg-surface-container">Kirim Card</button></div>
          </div>
        </section>
