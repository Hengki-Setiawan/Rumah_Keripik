  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-3">
        <div>{compactHeader ? <h2 className="font-headline-sm text-headline-sm text-on-surface">AI Skills</h2> : <h1 className="font-headline-lg text-headline-lg text-on-surface">AI Skills</h1>}<p className="mt-1 text-on-surface-variant">Peta kemampuan AI Rumah Keripik: tool, card, provider, dan guardrail.</p></div>
        <InfoButton title="AI Skills" description="Daftar kemampuan yang dapat dipakai asisten AI ketika melayani pelanggan, mulai rekomendasi produk sampai handoff admin." usage="Gunakan halaman ini untuk audit kemampuan aktif dan memahami input-output setiap skill sebelum mengubah rule atau model router." />
      </div>
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {skills.map((skill) => {
          const Icon = skill.icon;
          return <article key={skill.title} className="rounded-2xl border border-outline-variant bg-surface-container-lowest p-5"><div className="mb-4 flex items-center justify-between gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-primary-container text-primary"><Icon size={22} /></div><span className="rounded-full bg-green-50 px-2 py-0.5 text-[10px] font-medium text-green-700">{skill.status}</span></div><h2 className="font-headline-sm text-headline-sm text-on-surface">{skill.title}</h2><dl className="mt-4 space-y-2 text-sm"><Info label="Input" value={skill.input} /><Info label="Output" value={skill.output} /><Info label="Provider" value={skill.provider} /></dl></article>;
        })}
      </div>
    </div>
  );
