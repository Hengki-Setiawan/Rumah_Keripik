  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(135deg,#fff8e7,#ffe4ad,#f7c96e)] px-5 py-8 text-[#231305]">
      <section className="mx-auto max-w-4xl rounded-[2rem] border border-[#e0bd82] bg-white/90 p-6 shadow-2xl shadow-[#8d4b00]/15 md:p-10">
        ...
        {result && (
          <section className="mt-6 space-y-5 rounded-3xl border border-[#ecd3a7] bg-white p-5">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                <p className="text-sm font-bold text-[#735033]">Kode pesanan</p>
                <h2 className="text-3xl font-black">{result.order.kode_pesanan || result.order.id_transaksi}</h2>
            ...
            <div>
              ...
              <div className="mt-3 space-y-2">
                {result.events.slice(0, 8).map((event) => (
                  <div key={event.id} className="rounded-2xl border border-[#ecd3a7] bg-[#fffdf6] p-3 text-sm">
                    <b>{event.event_type.replace(/_/g, ' ')}</b>
                    <p className="text-xs text-[#735033]">{new Date(event.created_at).toLocaleString('id-ID')}</p>
                  </div>
                ))}
                {result.events.length === 0 && <p className="text-sm text-[#735033]">Timeline belum tersedia.</p>}
              </div>
            </div>
          </section>
        )}
...
function StatusCard({ label, value }: { label: string; value: string }) {
  return <div className="rounded-3xl bg-[#2a1606] p-5 text-white"><p className="text-sm font-bold text-white/60">{label}</p><p className="mt-2 break-words text-xl font-black">{value.replace(/_/g, ' ')}</p></div>;
}
