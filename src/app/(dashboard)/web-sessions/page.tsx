...
import { customerAddress, customerIdentity, customerProfile, transaksi, webOrderSession } from '@/lib/schema';
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4">
        {sessions.map(({ session, customer }) => {
          ...
          return (
            <section key={session.id_session} className="rounded-2xl border border-outline-variant bg-white p-5">
              <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div>
                  <p className="text-xs font-medium text-on-surface-variant">{session.status} - {session.current_state}</p>
                  <h2 className="mt-1 text-xl font-semibold tracking-[-0.02em]">{customer?.nama || 'Customer anonim'}</h2>
                ...
                <div className="text-left md:text-right">
                  {order ? <><p className="font-semibold">{order.kode_pesanan || order.id_transaksi}</p><p className="font-semibold text-primary">{formatRupiah(order.total_bayar)}</p><p className="text-sm text-on-surface-variant">{order.payment_status}</p></> : <p className="text-sm text-on-surface-variant">Belum ada order</p>}
                </div>
              </div>
              <details className="mt-4 rounded-xl bg-neutral-50 p-3">
                <summary className="cursor-pointer font-semibold">Context dan cart</summary>
        ...
        {sessions.length === 0 && <p className="rounded-2xl border border-outline-variant bg-white p-6 text-center text-on-surface-variant">Belum ada sesi web.</p>}
      </div>
      <CustomerSummary />
    </div>
  );
...
async function CustomerSummary() {
  ...
  return (
    <section className="rounded-2xl border border-outline-variant bg-white p-5">
      <h2 className="text-xl font-semibold tracking-[-0.02em]">Customer Web Terbaru</h2>
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        {customers.map((customer) => (
          <div key={customer.id_customer} className="rounded-xl bg-neutral-50 p-4">
            <p className="font-semibold">{customer.nama || customer.id_customer}</p>
