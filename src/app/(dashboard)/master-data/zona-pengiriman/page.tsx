  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-headline-sm text-headline-sm text-on-surface">Zona Pengiriman</h1>
          <p className="text-on-surface-variant font-body-md mt-1">Kelola zona pengiriman, ongkos kirim, dan radius coverage</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-surface-container-lowest border border-neutral-200 rounded-xl p-5 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              ...
              <input value={form.nama_zona} onChange={(e) => setForm({ ...form, nama_zona: e.target.value })}
            ...
            <div>
              ...
              <input value={form.lat_pusat} onChange={(e) => setForm({ ...form, lat_pusat: e.target.value })}
                placeholder="-0.5022" className="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-primary" required />
            </div>
            <div>
              <label className="block text-xs font-label-md text-on-surface-variant mb-1">Longitude Pusat *</label>
              <input value={form.lng_pusat} onChange={(e) => setForm({ ...form, lng_pusat: e.target.value })}
                placeholder="117.1536" className="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-primary" required />
            </div>
            <div>
              <label className="block text-xs font-label-md text-on-surface-variant mb-1">Radius (km)</label>
              <input type="number" value={form.radius_km} onChange={(e) => setForm({ ...form, radius_km: +e.target.value })}
                className="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-primary" min={1} />
            </div>
            <div>
              <label className="block text-xs font-label-md text-on-surface-variant mb-1">Ongkir Minimal</label>
              <input type="number" value={form.ongkir_min} onChange={(e) => setForm({ ...form, ongkir_min: +e.target.value })}
            ...
            <div>
              ...
              <input type="number" value={form.ongkir_max} onChange={(e) => setForm({ ...form, ongkir_max: +e.target.value })}
                className="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-primary" min={0} />
            </div>
          </div>
          ...
          {zonas.map((z) => (
            <div key={z.id} className={`bg-surface-container-lowest border border-neutral-200 rounded-xl overflow-hidden shadow-sm ${!z.is_active ? 'opacity-60' : ''}`}>
              ...
              <div className="h-40 bg-surface-container">
                {z.is_active && (
                  <MiniMap
                    lat={parseFloat(z.lat_pusat)}
                    lng={parseFloat(z.lng_pusat)}
                    height={160}
                    showGudang={true}
                  />
                )}
