...
const projectName = "rumah-keripik-admin";
...
async function ensurePagesProject() {
  ...
  if (dataGet.success) {
    ...
    if (patchData.success) {
      ...
      console.warn(`⚠️ Could not update env vars:`, patchData.errors);
    }
  } else {
...
async function prepareAssets() {
  ...
  const redirectsContent = `/pesan/* https://rumah-keripik.pages.dev/pesan 302
/pesan https://rumah-keripik.pages.dev/pesan 302
`;
  fs.writeFileSync(path.join(assetsDir, '_redirects'), redirectsContent);
  console.log(`✅ Created _redirects to cross-link /pesan to Storefront`);
}

async function main() {
  await ensurePagesProject();
  await prepareAssets();
  console.log(`\n🎉 Admin Pages assets and worker dependencies ready for deployment!`);
}
